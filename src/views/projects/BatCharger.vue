<template>
  <ProjectTemplate id="bat-charger" :media="media" :models="models">
    ...
    <n-p>
      ...
      I designed this board in order to test the concept first, before integrating it in other projects. That said i've used it in projects like 
      <router-link :to="{ name: 'comando-rita' }"><n-a>Rita's Garage Door Opener</n-a></router-link> and 
      <router-link :to="{ name: 'toDo' }"><n-a>Desk Vaccuum</n-a></router-link>, 
      and only recently, while integrating it into the <router-link :to="{ name: 'netscore' }"><n-a>Netscore</n-a></router-link>
      ...
      Not ideal, but easily fixed in the next revision (in Netscore's case i just rerouted the pcb traces with some jumper wires).
    </n-p>
    ...
    <n-ul>
      ...
      <n-li>Adjustable Current: <span class="squid-text-alt">Current can be adjusted with the use of a resistor.</span></n-li>
    ...
    <n-p>
      Here the main lesson to be taken is to double-check your work, even though the circuit seems to be working, the protection is being bypassed due to a routing mistake.
      Also the battery is never disconnected since we're not using mosfets in the OC/OD pins of the FS312MH, something to consider for the next revision.
    </n-p>
